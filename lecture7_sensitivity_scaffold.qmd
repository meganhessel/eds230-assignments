...
simple_erosion(I = 2, f = 4) # no erosion 
...
# PROBLEM: Sample is too small -> Tails are too different... Solution: generate a larger sample size. But sometimes pricey to run the model more. 
```
...
-   Latin Hypercube *(more accessible and common)*

-   Sobol *(the better method)*
...
-   2 parameters (x1 and x2) - vary at same time
...
p_names <- c("Tmincoeff1", "Tmincoeff2", "Pcoeff1", "Pcoeff2", "intercep")
...
parm <- as.data.frame(matrix(nrow = nrow(parm_quant), ncol = ncol(parm_quant))) # make empty maztrix 

colnames(parm_quant) <- p_names
...
pvar <- 15 # deivation of +/- 15% 

# for all parameters
# know mean from table and estimating/choosing +/- 15% sd 
...
parm[, "Pcoeff1"] = qnorm(parm_quant[, "Pcoeff1"], 
                          mean = -0.07, 
                          sd = 0.07 / pvar)
parm[, "Pcoeff2"] = qnorm(parm_quant[, "Pcoeff1"], 
                          mean = -0.0043, 
                          sd = 0.0043 / pvar)
parm[, "intercep"] = qunif(parm_quant[, "intercep"], # Example using dif distribution 
                           min = 0.28 - 0.28*pvar, 
                           max = 0.28 + 0.28*pvar)
...
# EACH ROW IS ALL INFORMATION I WANT TO RUN INTO MY HYPER CUBE!!! 
...
clim <- read.table(here::here("assignment3", "clim.txt"), 
           header = TRUE)
...
ggplot(yieldsd, aes(maxyield)) + stat_ecdf() # distrubtion of output 


# Which parameter is it senstive too 
tmp <- cbind.data.frame(yieldsd, parm) # have everything 

tmp2 <- tmp %>% 
  gather(maxyield, minyield, meanyield, value = "yvalue", key = "yieldtype")

tmp3 <- tmp2 %>% 
  gather(-yvalue, -yieldtype, # take out what you dont want to gather
...
tmp <- cbind.data.frame(yields, parm)
...
-   meanyield and minyield is more dominated by the precipitation
...
result <- map(parm, cor.test, y = yieldsd$maxyield) # `cor.test` calculates correlation coef
result$Tmincoeff1
result$Tmincoeff2
result$Pcoeff10
result$Pcoeff2
...
use latin hyper clubes to look at how parameter variation changes the outputs
