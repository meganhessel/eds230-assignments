...
height = runif(min = 9.5, 
              max = 10.5, 
              n = np)

# first set call it X1 - combind everything 
x1 <- cbind.data.frame(k_o, k_d, v, height)

# RE-RUN ALL THE PARMETERS then set x2
...
sens_Catm_sobol <- sobolSalt(model = NULL, # this input is super picky 
                             x1, x2, # give it the combined parameter df 
                             nboot = 100) # bootstrapping process 

# Take a look at the Sobol generated sensitivity object
str(sens_Catm_sobol) # Sobol object - look at X1, X2, and X (X = most important) 

  
# your parameters sets for sensitivity analysis are in X, look at that as well
parms = as.data.frame(sens_Catm_sobol$X) # extracts parameter values out 
colnames(parms) = colnames(x1) # give it same col names at x1 
...
res <- pmap_dbl(parms, Catm) # run model with all parameters just created 
...
sens_Catm_sobol$S # estimate for all parameters and confidence intervals 

# Tell you what parameters has the biggest effect on the output 
# Make sure to look at your confidence bounds - stability 
...
both <- cbind.data.frame(parms, gs = sens_Catm_sobol$X)
